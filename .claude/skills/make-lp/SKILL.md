...
7. **Responsive** — mobile-first; no horizontal scroll on narrow viewports except inside containers that are explicitly meant to scroll (e.g. a wide comparison table).
8. **No build step** — this page must run by opening the HTML file or serving the static folder as-is, independent of whatever bundler the app itself uses (Vite, etc.). Keep `main.js` as the readable source and hand-maintain a matching `main.min.js` — since there is no bundler for this folder, every edit to `main.js` must be mirrored into `main.min.js` by hand (minify manually, keep behavior identical). `<script src="main.min.js">` is what actually loads in the page.
